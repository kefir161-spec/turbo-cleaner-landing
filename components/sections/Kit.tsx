...
import { kit } from "@/content/product";
import { media } from "@/lib/assets";

export function Kit() {
  return (
    <Section id="kit" className="bg-ink-2">
      <SectionHeading eyebrow={kit.eyebrow} title={kit.title} index="06 / Комплект" />
        ...
        <Reveal className="lg:sticky lg:top-24">
          <div className="relative overflow-hidden rounded-[var(--radius-card)] border border-line bg-[radial-gradient(circle_at_50%_35%,rgba(255,255,255,0.08),transparent_70%)]">
            <div className="relative aspect-[1600/744]">
              <Image
                src={media.assembled}
                alt="Turbo Cleaner в сборе: пневмопистолет, штанга, конусный ограничитель и ротор"
                fill
            ...
            <div className="flex items-center justify-between border-t border-line px-5 py-3">
              <span className="mono-label">Изделие в сборе</span>
              <span className="font-mono text-xs text-accent">≈ 1 мин сборка</span>
            </div>
          </div>
          <div className="mt-4 grid grid-cols-2 gap-4">
            <div className="relative aspect-[1600/938] overflow-hidden rounded-2xl border border-line bg-[radial-gradient(circle_at_50%_40%,rgba(255,255,255,0.06),transparent_70%)]">
              <Image
                src={media.kitParts}
                alt="Сменные роторы, конусный ограничитель, пневмопистолет и штанга"
                fill
